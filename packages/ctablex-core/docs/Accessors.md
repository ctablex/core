...
const name: number = accessTo(product, 'name'); // ✗ Error! 'name' is not a number path
const tags: number = accessTo(product, (p) => p.tags); // ✗ Error! Returns string[], not number

// ✗ Type mismatch - path returns string, not number
const name: number = access(product, 'name'); // ✗ Error! Type 'string' is not assignable to 'number'
```
